...
//
// This hook went through two rounds of its own debate before release. The
...
// running the command in their own terminal; the hook only binds the agent.
...
try {
  ...
  // Unreadable input: only block if it looks PR-related, so a change in
  // the hook input format does not stop every command.
  if (/\bgh\b|pull_request|auto_merge/.test(raw))
    block("could not parse the hook input to check the debate record.");
  process.exit(0);
}
...
// --- Opt-in ------------------------------------------------------------------
const sessionDir =
  input.cwd && existsSync(input.cwd) ? input.cwd : process.env.CLAUDE_PROJECT_DIR || process.cwd();
